...
public class BuffMethods {
     ...
     * @param minion null
     * @param performBuff true if we want to perform a buff and false if we only
     *            want to check if it is possible to perform this buff
    ...
    /**
     * Method for performing buff: "Battlecry: If you're holding a Dragon, deal
     * 3 damage."
     * 
     * @param action the action that just took place
     * @param minion null
    ...
    /**
     * Method for performing buff: "Battlecry: Gain +1 Health for each card in
     * your hand."
     * 
     * @param action the action that just took place
     * @param minion null
    ...
    /**
     * Method for performing buff: "Combo: Gain +2/+2 for each card played
     * earlier this turn."
     * 
     * @param action the action that just took place
     * @param minion null
            ...
            if (m.getId().equals(action.getMinionCreatedId())) {
                if (performBuff) {
                    if (cardsPlayed.size() > 1) {
                        // must have two cards in the pile,
                        // where one of them is this one
    ...
    /**
     * Method for performing buff: "Battlecry: Gain +1 Attack for each other
     * card in your hand."
     * 
     * @param action the action that just took place
     * @param minion null
    ...
    /**
     * Method for performing buff: "After you cast a spell, deal 1 damage to ALL
     * minions."
     * 
     * @param action the action that just took place
     * @param minion the minion on the board that this buff belongs to
     * @param performBuff true if we want to perform a buff and false if we only
     *            want to check if it is possible to perform this buff
     * @return true if the buff was invoked or can be invoked, false otherwise
     */
    ...
    /**
     * Method for performing buff: "Battlecry: Set a hero's remaining Health to
     * 15."
     * 
     * @param action the action that just took place
     * @param minion null
    ...
    /**
     * Method for performing buff: "Whenever this minion takes damage, draw a
     * card."
     * 
     * @param action the action that just took place
     * @param minion the minion on the board that this buff belongs to
    ...
    public boolean drawCardWhenThisMinionTakesDamage(Action action, Minion minion, boolean performBuff) {
        ...
        if (action.getActionType().equals(Action.Type.DAMAGE)
                && minion.getId().equals(action.getDamagedCharacterId())) {
            for (Player p : action.getPlayers()) {
                for (Minion m : p.getActiveMinions()) {
    ...
    /**
     * Method for performing buff: "Whenever a player casts a spell, put a copy
    ...
    public boolean copyOpponentsPlayedSpellCardIntoHand(Action action, Minion minion, boolean performBuff) {
        ...
        for (Card c : ((GamePlayer) currentPlayer).getDiscardPileThisTurn()) {
            if (c.getId().equals(action.getPlayedCardId())) {
                if (c.getType().equals(Card.Type.SPELL)) {
                    if (performBuff) {
                        Card newCard = new FirestoneCard(UUID.randomUUID().toString(), c.getName(),
    ...
    /**
     * Method for performing buff: "Return a friendly minion to your hand. It
     * costs (2) less."
     * 
     * @param action the action that just took place
     * @param minion null
    ...
    public boolean returnOwnMinionToHand(Action action, Minion minion, boolean performBuff) {
        ...
        for (Minion m : currentPlayer.getActiveMinions()) {
            if (m.getId().equals(action.getTargetId())) {
                // target id is the id of the minion that player
                // wants to return to hand
    ...
    /**
     * Method for performing hero power: "Life Tap: Draw a card and take 2
     * damage."
     * 
     * @param action the action that just took place
     * @param minion null
    ...
    public boolean lifeTap(Action action, Minion minion, boolean performBuff) {
        if (!action.getActionType().equals(Action.Type.HERO_POWER) || minion != null || action.getTargetId() != null) {
            return false;
        }
        Player currentPlayer = getCurrentPlayer(action);
    ...
    /**
     * Method for performing hero power: "Steady Shot: Deal 2 damage to the
     * enemy hero."
     * 
     * @param action the action that just took place
     * @param minion null
